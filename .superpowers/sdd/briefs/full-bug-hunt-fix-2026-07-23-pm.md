...
| --- | --- | --- | --- |
| C1 | FIXED | Broadcast uses `send_and_confirm_transaction`; campaign drain runs only after confirmed real intent | `40e96c5` |
| I1 | FIXED | Campaign recompiles/signs each tx with a fresh blockhash immediately before simulate/send | `40e96c5` |
| I2 | FIXED | Handoff load passes `--rpc-url`; `localnet` requires localhost/127.0.0.1 (empty URL rejected) | `40e96c5` |
| I3 | FIXED | `fund_accounts` fetches a new blockhash before each funding transfer | `b34d758` |
| I4 | FIXED | `from_cooker_decoy_sink` requires `secret_key_path`; docs aligned | `cd4a336` (+ docs in `40e96c5`) |
...
Hunt/fix briefs commit: `7867939`.